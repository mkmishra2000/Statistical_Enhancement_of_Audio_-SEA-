function ReconstructedSignal=OverlapAdd2Noise(XNEW,yphase,windowLen,ShiftLen)
...
for i=1:FrameNum
    ...
    spec=Spec(:,i);
    sig(start:start+windowLen-1)=sig(start:start+windowLen-1)+real(fft(ifft(spec,windowLen)));    
end
